...
def _choose_representative_run(run_rows: list[dict[str, object]]) -> str:
    ...
    drift_detected = [row for row in run_rows if row["drift_detected"]]
    if drift_detected:
        return str(drift_detected[0]["run_id"])
...
def _benchmark_verdict(run_rows: list[dict[str, object]]) -> tuple[str, dict[str, object]]:
    ...
    informative = bool(drift_detected_rows) and bool(comparable_rows)
    if not informative:
...
def run_benchmark1(
    ...
    benchmark1_verdict, verdict_stats = _benchmark_verdict(run_rows)
    ...
    summary = {
        ...
        "benchmark1_verdict": benchmark1_verdict,
