...
The central idea is that these two kinds of signal do not always become visible on the same timescale. A direct symmetry observable may remain below threshold while a drift observable linked to the same breakdown process is already detectable in the time-series.
...
The novelty is not only that drift and symmetry are related. It is that drift may be the earlier and more practically useful observable under gradual symmetry breaking.
